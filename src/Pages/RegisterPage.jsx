...
function RegisterPage() {
    ...
    const RegisterhandleSubmitForm = async (values, setSubmitting) => {
      try {
        ...
        if (!isGoogle) {
          const response = await axios.post(`${BaseUrl}/account/register/`, values);
          if (response.status === 201) {
            toast.success('Registration completed successfully!');
            navigate("/login");
          }
        } else {
          const response = await axios.post(`${BaseUrl}/account/googleregister/`, values);
          if (response.status === 201) {
            setIsGoogle(false)
            console.log(response, 'google');
            const token = JSON.stringify(response.data.token);
            localStorage.setItem("token", token);
            toast.success(response.data?.msg || 'Registration compleated');
            navigate("/");
          }
        }
  ...
  return (
    <>
      ...
      >
        <Card
          shadow={true}
          className="border bg-opacity-90 px-10 py-20 sm:py-10"
        >
